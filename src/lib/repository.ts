...
async function readState():Promise<AppState>{ await fs.mkdir(dataDir,{recursive:true}); try{const state=JSON.parse(await fs.readFile(stateFile,"utf8")) as AppState;state.collections??=[];for(const collection of state.collections){collection.type??="document";for(const recipient of collection.recipients){recipient.drafts??=[];recipient.versions??=[]}}return state}catch(error){if((error as NodeJS.ErrnoException).code==="ENOENT")return {collections:[]};throw error} }
