...
installEmbedRuntime({
  ...
  handlers: {
    ...
    async getPendingAssignment() {
      await initPromise;
      return inputHandler?.getPendingAssignmentSelection() ?? null;
    },
    async applyPendingAssignment(assigneeId, assigneeName, fieldBaseName) {
      await initPromise;
      if (!inputHandler) throw new Error('편집기가 준비되지 않았습니다.');
      return inputHandler.applyPendingAssignment(assigneeId, assigneeName, fieldBaseName);
    },
    async cancelPendingAssignment() {
      await initPromise;
      inputHandler?.cancelPendingAssignment();
    ...
    async setEditableFieldSourceNames(sourceNames) {
      await initPromise;
      if (!inputHandler) throw new Error('편집기가 준비되지 않았습니다.');
      inputHandler.setEditableFormFieldSourceNames(sourceNames);
      return { sourceNames: inputHandler.getEditableFormFieldSourceNames() };
    },
    async getFieldValueBySourceName(sourceName) {
      await initPromise;
      if (!inputHandler) throw new Error('편집기가 준비되지 않았습니다.');
      return inputHandler.getEditableFormFieldValueBySourceName(sourceName);
    },
    async setFieldValueBySourceName(sourceName, value) {
      await initPromise;
      if (!inputHandler) throw new Error('편집기가 준비되지 않았습니다.');
      return inputHandler.setEditableFormFieldValueBySourceName(sourceName, value);
    },
    async listFields() {
      await initPromise;
      if (!inputHandler) throw new Error('편집기가 준비되지 않았습니다.');
      return inputHandler.listEditableFormFields();
    },
  },
});
