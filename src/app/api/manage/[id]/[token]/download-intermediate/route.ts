...
export async function GET(_request:Request,{params}:{params:Promise<{id:string;token:string}>}){
  try{const {id,token}=await params,item=await findManaged(id,tokenHash(token));if(!item||collectionMode(item)!=="shared_fields")return Response.json({error:"권한이 없거나 요청을 찾을 수 없습니다."},{status:404});const bytes=applySharedFieldValues(new Uint8Array(await readStored(item.templateStorageKey)),item.sharedFields??[],item.sharedFieldStates??[],true),name=safeFileName(`${item.title}_중간취합본_${timestamp()}.hwpx`);return new Response(Buffer.from(bytes),{headers:{"Content-Type":"application/vnd.hancom.hwpx","Content-Disposition":`attachment; filename*=UTF-8''${encodeURIComponent(name)}`,"Cache-Control":"private, no-store"}})}catch(error){console.error(error);return Response.json({error:"중간 취합본을 만들지 못했습니다."},{status:500})}
}
