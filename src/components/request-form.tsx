...
export function RequestForm({teachers}:{teachers:Teacher[]}){
  ...
  return <form onSubmit={submit} className="card card-pad form-grid">
    {handoff&&<div className="success full" style={{gridColumn:"1/-1"}}>빈 양식 만들기에서 넘어온 파일을 넣어 두었습니다. 제목과 마감일만 정하면 됩니다.</div>}
    ...
    {type!=="table"&&<div className="field"><label>{type==="document"?"작성 양식":"엑셀 양식"}</label><FileDrop key={type} name="template" required accept={type==="document"?".hwp,.hwpx":".xlsx"} hint={type==="document"?"HWP · HWPX":"XLSX"} initial={handoff?.template}/><span className="help">{type==="document"?'HWP/HWPX, 최대 20MB · HWPX에서 {{교사명}}, {{부서명}} 자동 치환':"XLSX, 최대 20MB · 내려받아 Excel에서 작성 후 제출"}</span></div>}
    {type==="document"&&<div className="field"><label>작년 자료 (선택)</label><FileDrop name="reference" accept=".hwp,.hwpx" hint="작년 완성본" initial={handoff?.reference??undefined}/><span className="help">함께 올리면 제출 화면에서 작년 문서를 왼쪽에 나란히 띄워 복사·붙여넣기 할 수 있습니다.</span></div>}
