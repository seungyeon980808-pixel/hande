...
export async function POST(request:Request,{params}:{params:Promise<{token:string}>}){
  try{
    ...
    if(collectionMode(item)==="shared_fields")return Response.json({error:"지정 필드 공동작성은 필드 저장 주소를 사용하세요."},{status:400});
