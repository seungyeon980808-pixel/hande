...
export function onKeyDown(this: any, e: KeyboardEvent): void {
  ...
  // 임베드 공동작성 배정 명령: 일반 클릭·드래그는 그대로 두고 `/`에서만
  // 현재 커서/텍스트/셀 블록을 고정한다. 두 번째 `/`는 호스트가 취소한 뒤 일반 입력한다.
  if (!e.isComposing && e.keyCode !== 229 && e.code === 'Slash'
      && !e.ctrlKey && !e.metaKey && !e.altKey && !e.shiftKey) {
    const selection = this.captureAssignmentSelection?.();
    if (selection) {
      e.preventDefault();
      return;
    }
