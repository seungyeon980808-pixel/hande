...
export function RequestForm({teachers}:{teachers:Teacher[]}){
  ...
  return <form onSubmit={submit} className="card card-pad form-grid">
    ...
    {type==="document"&&mode==="shared_fields"?<SharedFieldBuilder teachers={selectedTeachers}/>:type!=="table"&&<div className="field full"><label htmlFor="template">{type==="document"?"작성 양식":"엑셀 양식"}</label><input key={`${type}-${mode}`} id="template" name="template" required type="file" accept={type==="document"?".hwp,.hwpx":".xlsx"}/><span className="help">{type==="document"?'HWP/HWPX, 최대 20MB · HWPX에서 {{교사명}}, {{부서명}} 자동 치환':"XLSX, 최대 20MB · 내려받아 Excel에서 작성 후 제출"}</span></div>}
