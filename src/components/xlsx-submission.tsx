...
export function XlsxSubmission({token,person,templateName}:{token:string;person:Person;templateName:string}){
  ...
  useEffect(()=>{let active=true;const keyName=`school-work-xlsx-draft:${token}:${person.id}`;let key=localStorage.getItem(keyName);if(!key){key=crypto.randomUUID();localStorage.setItem(keyName,key)}draftKey.current=key;(async()=>{try{const response=await fetch(`/api/collect/${token}/draft?teacherId=${encodeURIComponent(person.id)}&draftKey=${encodeURIComponent(key)}`);if(response.status===204){if(active)setStatus("저장된 임시 파일이 없습니다.");return}if(!response.ok)throw new Error((await response.json()).error);const blob=await response.blob();if(active){showDraft(blob,decodeURIComponent(response.headers.get("X-Document-Name")||"임시저장.xlsx"));setStatus(`임시 파일이 저장되어 있습니다${response.headers.get("X-Draft-Updated-At")?` · ${new Date(response.headers.get("X-Draft-Updated-At")!).toLocaleString("ko-KR")}`:""}`)}}catch(cause){if(active)setError(cause instanceof Error?cause.message:"임시 파일을 확인하지 못했습니다.")}})();return()=>{active=false;if(draftObjectUrl.current)URL.revokeObjectURL(draftObjectUrl.current);draftObjectUrl.current=""}},[person.id,showDraft,token]);
  async function upload(mode:"draft"|"submit"){
    if(!file){setError("작성한 XLSX 파일을 선택하세요.");return}
    setBusy(true);setError("");
    try{const selectedFile=file;const form=new FormData();form.set("teacherId",person.id);form.set("draftKey",draftKey.current);form.set("document",selectedFile);const response=await fetch(`/api/collect/${token}/${mode==="draft"?"draft":"submissions"}`,{method:"POST",body:form});const body=await response.json();if(!response.ok)throw new Error(body.error);if(mode==="draft"){showDraft(selectedFile,selectedFile.name);setStatus(`임시 업로드 완료 · ${new Date(body.updatedAt).toLocaleString("ko-KR")}`)}else{setStatus(`${body.version}차 최종 제출 완료 · 기존 제출은 그대로 보관됩니다.`);clearDraft()}setFile(null)}catch(cause){setError(cause instanceof Error?cause.message:"파일을 올리지 못했습니다.")}finally{setBusy(false)}
  }
  return <div className="card card-pad xlsx-panel">
    <div className="xlsx-step"><span>1</span><div><strong>양식 내려받기</strong><p>담당자가 올린 원본 양식을 Excel에서 작성하세요.</p><a className="btn btn-secondary" href={`/api/collect/${token}/template?teacherId=${encodeURIComponent(person.id)}`}>{templateName} 다운로드</a></div></div>
    <div className="xlsx-step"><span>2</span><div><strong>작성 파일 선택</strong><p>작성한 XLSX 파일을 선택하세요. 최대 20MB까지 가능합니다.</p><input type="file" accept=".xlsx" onChange={event=>setFile(event.target.files?.[0]??null)}/>{file&&<p className="help">선택: {file.name}</p>}</div></div>
