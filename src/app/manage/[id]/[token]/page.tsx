...
export default async function ManagePage({params}:{params:Promise<{id:string;token:string}>}){
  ...
  return <AppFrame>
    ...
    <div className="page-head"><div><h1>{item.title}</h1><p className="subtle">{collectionTypeLabel(type)} · 마감 {new Date(item.deadline).toLocaleString("ko-KR")} · 관리 링크</p></div><div className="page-actions">{type==="table"&&<a className="btn btn-primary" href={`/api/manage/${id}/${token}/export-table`}>통합 XLSX 다운로드</a>}{type==="xlsx"&&<a className="btn btn-primary" href={`/api/manage/${id}/${token}/download-latest`}>최신 제출 ZIP</a>}<ManageAutoRefresh/></div></div>
...
function SharedFieldsManage({item,id,token}:{item:Collection;id:string;token:string}){
  ...
  return <AppFrame>
    <ManageAccessCookie id={id} token={token}/>
    <div className="page-head"><div><h1>{item.title}</h1><p className="subtle">지정 필드 공동작성 · 마감 {new Date(item.deadline).toLocaleString("ko-KR")} · 발행 후 설정 고정</p></div><div className="page-actions"><a className="btn btn-secondary" href={`/api/manage/${id}/${token}/download-intermediate`}>중간 HWPX</a><a className="btn btn-primary" href={`/api/manage/${id}/${token}/download-final`}>최종 HWPX</a><ManageAutoRefresh/></div></div>
