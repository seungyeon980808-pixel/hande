...
const GROUPS=[
  ...
  {kind:"날짜",title:"날짜",hint:"해가 바뀌면 요일이 달라집니다. 비우고 새 학사일정에 맞춰 적으세요."},
  {kind:"이름",title:"담당자 이름",hint:"담당자가 바뀌지 않았다면 체크를 해제하세요."},
] as const;
  ...
  return <div style={{display:"grid",gap:16}}>
    ...
    {analysis&&analysis.items.length>0&&<div className="card card-pad">
      ...
      {GROUPS.map(group=>{
        ...
        const fills=group.kind==="날짜"||group.kind==="이름";
        return <section key={group.kind} style={{marginTop:18}}>
            ...
            {items.map(item=><label key={item.id} className="teacher-check" style={{cursor:"pointer",textAlign:"left",alignItems:"center"}}>
              ...
              <span style={{flex:1,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                ...
                {item.options
                  ...
                  :<code style={{background:"#eef1f5",padding:"2px 8px",borderRadius:4,color:"#607080"}}>비움</code>}
                {item.count>1&&<span className="help">문서에 {item.count}곳</span>}
              </span>
            </label>)}
          </div>
          {fills&&<p className="help" style={{marginTop:6}}>{group.kind==="이름"?"그대로 두려면 체크를 해제하세요.":"비우지 않으려면 체크를 해제하세요."}</p>}
        </section>;
      })}
