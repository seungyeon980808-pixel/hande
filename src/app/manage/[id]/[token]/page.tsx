...
export default async function ManagePage({params}:{params:Promise<{id:string;token:string}>}){
  ...
  return <AppFrame>
    <div className="page-head"><div><h1>{item.title}</h1><p className="subtle">{collectionTypeLabel(type)} · 마감 {formatDateTime(item.deadline)} · 관리 링크</p></div><div className="page-actions">{type==="table"&&<a className="btn btn-primary" href={`/api/manage/${id}/${token}/export-table`}>통합 XLSX 다운로드</a>}{type==="xlsx"&&<a className="btn btn-primary" href={`/api/manage/${id}/${token}/download-latest`}>최신 제출 ZIP</a>}<Link className="btn btn-secondary" href={`/manage/${id}/${token}`}>현황 새로고침</Link></div></div>
