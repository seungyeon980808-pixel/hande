...
export function SharedFieldsEditor({token,person}:{token:string;person:Person}){
  ...
  useEffect(()=>{let active=true,instance:FormEditor|null=null;(async()=>{try{
    ...
    if(!active)return;
    // 담당 칸이 문서 아래쪽이면 음영을 칠해도 열자마자는 보이지 않는다. 첫 칸까지 데려다 준다.
    if(next.length)await instance._request("scrollToFirstEditableField").catch(()=>{});
    const values=await readValues();lastValues.current=Object.fromEntries(values.map(value=>[value.fieldId,value.value]));setReady(true);setMessage(next.length?`노란색으로 표시된 ${next.length}군데가 내가 작성할 곳입니다.`:"현재 본인에게 지정된 작성 영역이 없습니다.");
