...
type AssignmentSelection =
  | { id: number; kind: 'caret'; position: DocumentPosition; summary: string }
  | { id: number; kind: 'text'; start: DocumentPosition; end: DocumentPosition; summary: string }
  | { id: number; kind: 'cells'; sec: number; ppi: number; ci: number; cellIndices: number[]; cellPath?: DocumentPosition['cellPath']; summary: string };
...
export class InputHandler {
  ...
  /** Embed host가 지정한 참여자용 ClickHere 필드 원본 이름. 기본값(빈 목록)은 모두 잠근다. */
  private editableFormFieldSourceNames = new Set<string>();
  private editableFormFieldIds = new Set<number>();
  ...
  */
  ...
  getEditableFormFieldSourceNames(): string[] {
    return [...this.editableFormFieldSourceNames];
  }
  ...
  /** 허용 목록에 있는 ClickHere 필드만 이름 기반 RPC로 읽거나 쓸 수 있다. */
  getEditableFormFieldValueBySourceName(sourceName: string): { sourceName: string; fieldId: number; value: string } {
    const field = this.getAllowedClickHereFieldBySourceName(sourceName);
    const result = this.wasm.getFieldValue(field.fieldId);
    if (!result.ok) throw new Error('필드 값을 읽지 못했습니다.');
    return { sourceName, fieldId: field.fieldId, value: result.value };
  }

  setEditableFormFieldValueBySourceName(sourceName: string, value: string): { sourceName: string; fieldId: number; oldValue: string; newValue: string } {
    if (this.editMode !== 'form') throw new Error('필드 값 변경은 양식 모드에서만 가능합니다.');
    const field = this.getAllowedClickHereFieldBySourceName(sourceName);
  ...
  listEditableFormFields(): Array<{ sourceName: string; fieldId: number; value: string }> {
    return this.wasm.getFieldList()
      .filter((field) => field.fieldType === 'clickhere' && this.editableFormFieldSourceNames.has(field.name))
      .map((field) => ({ sourceName: field.name, fieldId: field.fieldId, value: field.value }));
  }
  ...
  private isEditableFormFieldPosition(pos: DocumentPosition): boolean {
    ...
    if (!fi?.editableInForm || !this.isAllowedClickHereFieldInfo(fi)) return false;
    const start = fi.startCharIdx ?? -1;
    const end = fi.endCharIdx ?? -1;
    return pos.charOffset >= start && pos.charOffset <= end;
  }

  private isAllowedClickHereFieldInfo(fi: { fieldId?: number }): boolean {
    return typeof fi.fieldId === 'number' && this.editableFormFieldIds.has(fi.fieldId);
  }

  private getAllowedClickHereFieldBySourceName(sourceName: string): { fieldId: number; name: string } {
    if (!this.editableFormFieldSourceNames.has(sourceName)) throw new Error('편집이 허용되지 않은 필드입니다.');
    const field = this.wasm.getFieldList().find((candidate) =>
      candidate.fieldType === 'clickhere' && candidate.name === sourceName,
    );
    if (!field) throw new Error('허용된 ClickHere 필드를 찾지 못했습니다.');
    return field;
  }
  ...
  canDeleteSelectionInFormMode(): boolean {
    ...
    if (!endInfo?.editableInForm || !this.isAllowedClickHereFieldInfo(endInfo) || endInfo.fieldId !== fi.fieldId) return false;
    const start = fi.startCharIdx ?? -1;
    const end = fi.endCharIdx ?? -1;
  ...
  moveToAdjacentFormField(delta: number): boolean {
    ...
    const fields = this.wasm.getFieldList()
      .filter((field: any) =>
        ...
        && this.editableFormFieldSourceNames.has(field.name)
  ...
  private pendingAssignmentSelection: AssignmentSelection | null = null;
  private assignmentSelectionSerial = 0;

  /** `/` 명령 시 현재 커서·텍스트·셀 범위를 호스트 배정 대기 상태로 고정한다. */
  captureAssignmentSelection(): AssignmentSelection | null {
    if (this.cursor.isInHeaderFooter() || this.cursor.isInFootnote() || this.cursor.isInTableObjectSelection()) return null;
    const id = ++this.assignmentSelectionSerial;
    const block = this.getSelectedCellBlock();
    if (block && block.cellIndices.length > 0) {
      this.pendingAssignmentSelection = {
        id, kind: 'cells', sec: block.sec, ppi: block.ppi, ci: block.ci,
        cellIndices: [...block.cellIndices], cellPath: block.cellPath,
        summary: `표 셀 ${block.cellIndices.length}개`,
      };
      return structuredClone(this.pendingAssignmentSelection);
    }
    const selection = this.getNonEmptySelection();
    if (selection) {
      this.pendingAssignmentSelection = { id, kind: 'text', start: selection.start, end: selection.end, summary: '선택한 텍스트' };
      return structuredClone(this.pendingAssignmentSelection);
    }
    const position = this.cursor.getPosition();
    this.pendingAssignmentSelection = {
      id, kind: 'caret', position,
      summary: position.parentParaIndex === undefined ? '현재 본문 커서' : '현재 표 셀 커서',
    };
    return structuredClone(this.pendingAssignmentSelection);
  }

  getPendingAssignmentSelection(): AssignmentSelection | null {
    return this.pendingAssignmentSelection ? structuredClone(this.pendingAssignmentSelection) : null;
  }

  cancelPendingAssignment(): void { this.pendingAssignmentSelection = null; }

  /** 저장된 배정 위치를 실제 이름 있는 누름틀로 변환한다. */
  applyPendingAssignment(assigneeId: string, assigneeName: string, fieldBaseName: string): { ok: true; count: number; fieldNames: string[] } {
    const pending = this.pendingAssignmentSelection;
    if (!pending) throw new Error('배정할 위치가 없습니다. 문서에서 위치를 선택하고 /를 눌러 주세요.');
    const fieldNames: string[] = [];
    this.executeOperation({
      kind: 'snapshot', operationType: 'assignSharedFields', operation: (wasm) => {
        const insertAt = (position: DocumentPosition, name: string, initialValue = '') => {
          const result = wasm.insertClickHereField(position, `${assigneeName} 작성`, '', name, true);
          if (!result.ok) throw new Error('작성 영역을 삽입하지 못했습니다.');
          if (initialValue) wasm.setFieldValueByName(name, initialValue);
          fieldNames.push(name);
        };
        if (pending.kind === 'cells') {
          if (pending.cellPath && pending.cellPath.length > 1) throw new Error('중첩 표의 여러 셀 배정은 아직 지원하지 않습니다.');
          pending.cellIndices.forEach((cellIndex, index) => {
            const paragraphs = wasm.getCellParagraphCount(pending.sec, pending.ppi, pending.ci, cellIndex);
            const values: string[] = [];
            for (let para = 0; para < paragraphs; para++) {
              const length = wasm.getCellParagraphLength(pending.sec, pending.ppi, pending.ci, cellIndex, para);
              values.push(length > 0 ? wasm.getTextInCell(pending.sec, pending.ppi, pending.ci, cellIndex, para, 0, length) : '');
            }
            for (let para = paragraphs - 1; para >= 0; para--) {
              const length = wasm.getCellParagraphLength(pending.sec, pending.ppi, pending.ci, cellIndex, para);
              if (length > 0) wasm.deleteTextInCell(pending.sec, pending.ppi, pending.ci, cellIndex, para, 0, length);
            }
            const name = `${fieldBaseName}_${index + 1}`;
            insertAt({ sectionIndex: pending.sec, paragraphIndex: pending.ppi, parentParaIndex: pending.ppi, controlIndex: pending.ci, cellIndex, cellParaIndex: 0, charOffset: 0 }, name, values.join('\n'));
          });
        } else if (pending.kind === 'text') {
          const { start, end } = pending;
          const sameBodyParagraph = start.parentParaIndex === undefined && end.parentParaIndex === undefined
            && start.sectionIndex === end.sectionIndex && start.paragraphIndex === end.paragraphIndex;
          const sameCellParagraph = start.parentParaIndex !== undefined && end.parentParaIndex !== undefined
            && start.sectionIndex === end.sectionIndex && start.parentParaIndex === end.parentParaIndex
            && start.controlIndex === end.controlIndex && start.cellIndex === end.cellIndex
            && start.cellParaIndex === end.cellParaIndex;
          if (!sameBodyParagraph && !sameCellParagraph) throw new Error('여러 문단에 걸친 텍스트 선택은 아직 배정할 수 없습니다.');
          const count = Math.max(0, end.charOffset - start.charOffset);
          let initialValue = '';
          if (sameCellParagraph) {
            initialValue = wasm.getTextInCell(start.sectionIndex, start.parentParaIndex!, start.controlIndex!, start.cellIndex!, start.cellParaIndex!, start.charOffset, count);
            wasm.deleteTextInCell(start.sectionIndex, start.parentParaIndex!, start.controlIndex!, start.cellIndex!, start.cellParaIndex!, start.charOffset, count);
          } else {
            initialValue = wasm.getTextRange(start.sectionIndex, start.paragraphIndex, start.charOffset, count);
            wasm.deleteText(start.sectionIndex, start.paragraphIndex, start.charOffset, count);
          }
          insertAt(start, fieldBaseName, initialValue);
        } else {
          insertAt(pending.position, fieldBaseName);
        }
        return pending.kind === 'caret' ? pending.position : pending.kind === 'text' ? pending.start : this.cursor.getPosition();
      },
    });
    this.pendingAssignmentSelection = null;
    return { ok: true, count: fieldNames.length, fieldNames };
  }
  ...
  removeCurrentField(posOverride?: DocumentPosition): void {
    ...
    try {
      // 양식 모드에서는 누름틀 자체를 제거할 수 없다. 허용 목록은 값 편집에만 적용한다.
      if (this.editMode === 'form') return;
      this.cursor.clearSelection();
      this.executeOperation({
        kind: 'snapshot',
